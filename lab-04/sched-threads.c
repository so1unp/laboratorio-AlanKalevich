...
int main(int argc, char *argv[])
{
    ...
    if (argc != 4)
    {
        fprintf(stderr, "Uso: %s [-R | -F | -C] hilos items\n", argv[0]);
    ...
    {
        fprintf(stderr, "Error: opción invalida %s.\nUsar -R (SCHED_RR) o -F (SCHED_FIFO) o -C (SCHED_OTHER)\n", argv[1]);
        exit(EXIT_FAILURE);
    }
    ...
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);   // Inicializa el conjunto de CPU
    CPU_SET(0, &cpuset); // Agrega el primer núcleo al conjunto
    ...
    {
        pthread_t h;
        pthread_create(&h, &attr, write_buffer, (void *) i);
        hilos[i] = h;
    }
